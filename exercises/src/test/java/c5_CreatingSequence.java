...
public class c5_CreatingSequence {
    ...
    public void optional_value() {
        ...
        Mono<String> optionalMono = Mono.just(optionalValue.get());
    ...
    public void generate_programmatically() {
        ...
        AtomicInteger runnableCounter = new AtomicInteger(1);
        Flux<Integer> generateFlux = Flux.generate(sink -> {
            if(runnableCounter.get() > 5 ){
                sink.complete();
            }
            sink.next(runnableCounter.getAndIncrement());
        });
